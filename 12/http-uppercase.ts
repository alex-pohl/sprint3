...
const server = http.createServer((req: any, res: any) => { // TIPOS no consigo que funcionen
    if (req.method === 'POST'){ //solo aplicamos la lógica a las llamas tipo POST
        req.pipe(map((datos: string) => datos.toString().toUpperCase())).pipe(res); // Recibimos los datos, los metemos en MAP (como un array) como string en MAYUS, todo eso se devuelve a la respuesta (res).
    }
});
